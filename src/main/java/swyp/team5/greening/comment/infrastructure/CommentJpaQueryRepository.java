...
public interface CommentJpaQueryRepository extends JpaRepository<Comment, Long>,
    ...
    @Override
    @Query("""
           SELECT new swyp.team5.greening.comment.dto.response.FindMyAllCommentResponseDto(
               comment.id, comment.comment, post.id, post.categoryId,
               post.title, comment.createdAt, comment.updatedAt)
           FROM Comment comment
           INNER JOIN Post post
